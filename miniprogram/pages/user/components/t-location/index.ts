...
Component({
  ...
  methods: {
      ...
      getPermission({ code: 'scope.address', name: '通讯地址' }).then(() => {
        wx.chooseAddress({success: async (options) => {
            ...
            try {
              ...
              const params = Object.assign(target, { provinceCode,cityCode,districtCode});
    ...
    async queryAddress(addressId:AnyArray) {
      try {
